  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="text-center space-y-2">
        <div className="inline-flex items-center space-x-2 rtl:space-x-reverse bg-gradient-to-r from-teal-500 to-blue-600 text-white px-4 py-2 rounded-full">
      ...
      <Alert type="info" title={t('medicalDisclaimer')} message={t('medicalDisclaimer')} />
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 transition-colors duration-300">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <textarea id="consultation-query" value={query} onChange={(e) => setQuery(e.target.value)} placeholder={t('consultationPlaceholder')} rows={4} className="w-full p-4 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-teal-500 dark:bg-gray-700 dark:text-white resize-none" disabled={isLoading} />
          </div>
          <button type="submit" disabled={!query.trim() || isLoading} className="w-full bg-gradient-to-r from-teal-500 to-blue-600 text-white font-semibold py-3 px-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 disabled:opacity-50 flex items-center justify-center space-x-2 rtl:space-x-reverse">
            {isLoading ? (<LoadingSpinner size="sm" className="text-white" />) : (<><Send className="w-5 h-5" /><span>{t('getConsultation')}</span></>)}
          </button>
        </form>
        {user && (
      ...
      {error && (<Alert type="error" title="Consultation Error" message={error} />)}
