  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="text-center space-y-2">
        <div className="inline-flex items-center space-x-2 rtl:space-x-reverse bg-gradient-to-r from-pink-500 to-rose-600 text-white px-4 py-2 rounded-full">
          <Heart className="w-5 h-5" />
          <span className="font-medium">{t('wellnessTitle')}</span>
        </div>
        <p className="text-gray-600 dark:text-gray-400">{t('wellnessDesc')}</p>
      </div>

      <Alert type="info" title={t('wellnessGuidance')} message={t('wellnessGuidanceMessage')} />
      ...
      {error && (<Alert type="error" title="Planning Error" message={error} />)}
